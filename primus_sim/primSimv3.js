...
function TryToKillShip_( targetedShip, simNumber, isCounter, isSATK) {
  ...
  if (fleet[targetedShip].isDead == true) {
    ...
    if (isCounter == true) {    // Primus counterattack - guaranteed hit, we cannot respond, invisibility doesn't matter
      ...
      if (fleet[targetedShip].stasis.isDeathProof == false) {
        isDead = true;
      }
      else {
      ...
      if (fleet[targetedShip].invis.isInvisible == true) {
        ...
        rnd = Math.random()*100;
        if (rnd < fleet[targetedShip].dodgeRate) {  // Primus missed!
          isDead = false;
        }
        else {
          // Successful hit
          ...
          if (fleet[targetedShip].stasis.isDeathProof == false) {
            ...
            // Revoke invincibility buff as it has been consumed
            fleet[targetedShip].stasis.isDeathProof = false;
            fleet[targetedShip].stasis.turnsLeft = 0;
            fleet[targetedShip].accumulator += (targetedShip=="alfred") ? 0 : 25;
            ...
            if ( isSATK == false ) {
              ...
              if (rnd <= currentBlockRate) {
                ...
                rnd = Math.random()*100;
                if (rnd <= fleet[targetedShip].critChance) damage = damage*fleet[targetedShip].critDam/100;
...
function DoAttackV3_( attacker, simNumber) {
  var rnd = 0,    shotBlocked = false,    shotCountered = false,    isSatk = false,
  ...
  // Otherwise, decrement the value of tempTurns.
  ...
  if (rnd > attacker.hitRate) {
    ...
    rnd = Math.random()*100;
    if (rnd <= attacker.critChance) damage = damage*(attacker.critDam/100);
